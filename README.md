...
- **Colab 在线运行** -- 所有教程笔记本支持一键在 Google Colab 中打开，配合 Gemini 辅助调试
- **现代工程规范** -- src layout、pyproject.toml、ruff lint、pytest 测试、CI/CD

## 在线运行 | Run on Colab

无需本地安装，直接在 Google Colab 中运行教程笔记本。在 Colab 中还可以借助 Gemini 辅助调试，修改后直接保存回 GitHub 仓库。

No local installation needed -- run tutorials directly in Google Colab. You can also use Gemini in Colab for debugging, and save changes back to GitHub.
...
```
打开 Colab 笔记本 → 运行代码 → Gemini 辅助调试 → File > Save a copy in GitHub → 提交修改
```
...
点击上方 Colab 链接即可，notebook 首个 cell 会自动安装：
...
| sinc 定义 | `sin(pi*x)/(pi*x)` | `sin(pi*x)/(pi*x)` | 归一化 sinc（与 NumPy 一致） |
| 索引 | 1-based | 0-based | Python 标准 |
| 返回类型 | MATLAB matrix | `numpy.ndarray` | Python 标准 |

## 项目来源 | Origin

本项目是 [spaitlab/pyspt](https://github.com/spaitlab/pyspt) 的 v2 重构版本。原始仓库包含 157 个 Jupyter 教学笔记本和 54 个分散的 Python 脚本，v2 版将这些功能整合为标准的可安装 Python 包。参考了 [ThinkDSP](https://github.com/AllenDowney/ThinkDSP) 的 Colab 在线教学模式。
